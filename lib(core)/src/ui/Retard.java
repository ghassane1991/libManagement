 ...
 * @author  user
 */
public class Retard extends Shell {
	private Table table;
	/**
	...
	public static void main(String args[]) {
		try {
			...
			Retard shell = new Retard(display);
	...
	public Retard(Display display) {
		...
		addShellListener(new ShellAdapter() {
			...
			public void shellActivated(ShellEvent e) {
				List<loansManagement.History> bl = lib.getRetard();
		...
		tblclmnCopyId.setWidth(167);
		tblclmnCopyId.setText("Copy ID");
		
		TableColumn tblclmnLoanDate = new TableColumn(table, SWT.NONE);
		tblclmnLoanDate.setWidth(122);
		tblclmnLoanDate.setText("Loan date");
		
		TableColumn tblclmnMustReturnDate = new TableColumn(table, SWT.NONE);
		tblclmnMustReturnDate.setWidth(115);
		tblclmnMustReturnDate.setText("Must return date");
